...
<ParamField path="--kind" type="string">
  Filter: `note` (pins an agent picks up) or `comment` (notes people left for people — the
  toolbar's *Note* option or [`pinbox pin --comment`](/cli/commands/pin); the hub never
  delivers these to an agent). Default: both. In the table, an open comment pin shows
  `note` in the status column.
</ParamField>
...
| Exit | Code | When |
| --- | --- | --- |
| 2 | `E_INVALID_INPUT` | `--status` is not `open`/`resolved`, or `--kind` is not `note`/`comment` |
