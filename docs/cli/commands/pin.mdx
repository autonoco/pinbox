...
Options:
  ...
  --model-anchor <json> a revision-qualified 3D model/part anchor
  --author-json <json>  asserted identity supplied by a trusted backend
...
<ParamField path="--comment" type="boolean">
  A note for people, not a task. The pin is stored with `kind: "comment"`; the hub never
  routes it to an agent session, and the agent skill tells agents to read such pins as
  context and never act on or resolve them. The toolbar's *Note* option creates the same
  kind. Filter them with [`pinbox list --kind comment`](/cli/commands/list).
</ParamField>
...
Use `--model-anchor <json>` to attach the pin to a part-local 3D point. A trusted
backend can supply its verified user with `--author-json <json>`. See
[3D model pins](/concepts/3d-pins) for the schema, security boundary, and viewer integration.
