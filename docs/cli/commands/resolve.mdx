...
`--link`. Run it once the change is made and verified. Resolving is once-only: a second
resolve of the same pin is a conflict, not a no-op.
...
<ParamField path="id" type="string">
  Pin id (`pin_xxxxxxxxxx`). Ids are exact — prefixes do not match. Required unless
  `--link` is given.
</ParamField>

## Options

<ParamField path="--link" type="connector#ref">
  Resolve every open pin linked to this tracker item instead of one by id — the label
  [`pinbox link`](/cli/commands/link) prints, e.g. `github#58`. The default note is
...
```console
$ pinbox resolve --link github#58 --as agent
pin_6dzjtpax70 resolved
pin_kgoksic8rz resolved
2 pin(s) resolved for github#58
```

Reviewers have the same action in the toolbar's inbox, where pins linked to one item are
grouped under its header.
...
| Exit | Code | When |
| --- | --- | --- |
| 2 | `E_INVALID_INPUT` | neither `id` nor `--link` given; `--link` is not `connector#ref`; or `--as` is outside `human`/`agent` |
