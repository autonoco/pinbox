...
| `kind` | `note`, `comment` or `move`. Defaults to `note` (an agent picks it up). `comment` is a remark for people: the hub never delivers it to an agent session, and the toolbar shows it as a muted `N` chip. `move` carries `move: { from, to }` rects. |
