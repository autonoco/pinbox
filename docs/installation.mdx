...
| `PINBOX_VERSION` | `latest` | A release tag, e.g. `v0.14.0` |

```sh
curl -fsSL https://github.com/autonoco/pinbox/releases/latest/download/install.sh |
  env PINBOX_INSTALL_DIR=/usr/local/bin PINBOX_VERSION=v0.14.0 sh
```
...
For hacking on pinbox itself, or platforms with no prebuilt binary. You need
[Bun](https://bun.sh) 1.3 or newer — but only to build; the binary that comes
out does not need it.
...
A source build reports the version of the checkout you built, not the latest
release tag.

## Libraries

These are separate npm packages, versioned and released together with the CLI.
Install them into your app, not globally.
