...
the repo. Deploy the worker and you are done. It reads the hub's public URL from a
`custom_domain` route in the worker's `wrangler.jsonc`; on workers.dev it asks for it (or
pass `--hub`). Whether the App is created under an organization or a user follows the repo
owner's profile. `--print-secrets` sets the secrets yourself instead of through wrangler.
