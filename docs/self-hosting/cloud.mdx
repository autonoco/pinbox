...
| `GITHUB_APP_ID`, `GITHUB_INSTALLATION_ID`, `GITHUB_REPO` | var | The GitHub App that backs `pinbox link <id> github`. See [GitHub](/integrations/github#on-a-cloud-hub). |
| `GITHUB_API_BASE` | var | GitHub Enterprise Server API root, e.g. `https://ghe.example.com/api/v3`. Omit for github.com. |
...
| `GITHUB_APP_PRIVATE_KEY` | The GitHub App's PEM, as downloaded (PKCS#1) or PKCS#8 |
| `GITHUB_WEBHOOK_SECRET` | Verifies `POST /_pinbox/webhooks/github`; the route mounts only when this and the `GITHUB_REPO` var are both set |
| `SLACK_BOT_TOKEN` (with `SLACK_CHANNEL` var) | The Slack connector |
...
| `POST /_pinbox/webhooks/github` | GitHub App webhook; authenticated by its HMAC signature, not a bearer |
