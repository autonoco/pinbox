...
- **What screenshots capture.** By default the toolbar snapshots the pinned element's DOM
  (text and styles, no external images). With tab capture on, it captures the visible
  pixels of the element instead. Either way, if the element holds a customer record or a
  token in a debug panel, the screenshot includes it, and it goes wherever the pin goes.
